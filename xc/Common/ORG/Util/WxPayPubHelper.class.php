...

{
...

	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, C('WECHAT_PAY_SSLCERT_PATH'));

		//默认格式为PEM，可以注释

		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');

		curl_setopt($ch,CURLOPT_SSLKEY, C('WECHAT_PAY_SSLKEY_PATH'));
...

{
...

	{
	   	...
	   	$this->parameters["mch_id"] = C('WX_SHANGHUHAO');//商户号
...

{
...

	{
		...
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	...
		   	$this->returnParameters["mch_id"] = C('WX_SHANGHUHAO');//商户号
...

{
...

	{
	   ...
	   //import('Common.ORG.Util.WxPayPubHelper');

 	    //$weObj = new \Common\ORG\Util\JsApi_pub();
	...
	function createRefundUrlForCode($out_trade_no,$total_fee,$refund_fee)

	{
		
		$array["appid"] = C('WX_APPID');

		$array["mch_id"] = C('WX_SHANGHUHAO');

		$array["nonce_str"] = $this->createNoncestr();

		$array["op_user_id"] = C('WX_SHANGHUHAO');

		$array["out_refund_no"] = $this->createNoncestr();

		$array["out_trade_no"] = $out_trade_no;

		$array["total_fee"] = $total_fee;

		$array["refund_fee"] = $refund_fee;

		$cc = strtoupper(md5("appid=".C('WX_APPID')."&mch_id=".C('WX_SHANGHUHAO')."&nonce_str=".$array["nonce_str"]."&op_user_id=".C('WX_SHANGHUHAO')."&out_refund_no=".$array["out_refund_no"]."&out_trade_no=".$out_trade_no."&refund_fee=".$total_fee."&total_fee=".$refund_fee."&key=".C('WX_PAY_KEY')));

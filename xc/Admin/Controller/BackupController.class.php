...
class BackupController extends CommonController
{
    ...
    public function index_post(){
    	if (IS_POST || isset($_GET['dosubmit']))
    	{
    		...
    		} else
    		{
    			...
    			if (is_dir(APP_ROOT_PATH . $this->backup_path . $this->backup_name))
    			{
    				$this->error('备份名称已经存在');
    			}
    			mkdir(APP_ROOT_PATH . $this->backup_path . $this->backup_name);
    	         
				 
				if (!is_file(APP_ROOT_PATH . $this->backup_path . $this->backup_name .
    					'/tbl_queue.log'))
    			{
    				//写入队列
    ...
    {
        $sql_file = APP_ROOT_PATH . $this->backup_path . $this->backup_name . '/' . $sql_file_name;
    ...
    public function del_backup()
    {
        ...
        import('@.ORG.Dir'); 
        $dir=new \Dir();
        $dir->delDir(APP_ROOT_PATH . $this->backup_path . $_GET['backup'].'/');             
        $this->success('操作成功！');
    }
    ...
    public function download()
    {
        ...
        $file = isset($_GET['file']) && trim($_GET['file']) ? trim($_GET['file']) : $this->
            error('请选择备份文件！');
        $sql_file = APP_ROOT_PATH . $this->backup_path . $backup_name . '/' . $file;
    ...
    private function _get_vols($backup_name)
    {
        $vols = array(); //所有的卷
        $bytes = 0;
        $vol_path = APP_ROOT_PATH . $this->backup_path . $backup_name . '/';
        if (is_dir($vol_path))
        {
    ...
    {
        $backups = array(); //所有的备份
        if (is_dir(APP_ROOT_PATH . $this->backup_path))
        {
            if ($handle = opendir(APP_ROOT_PATH . $this->backup_path))
            {
                while (($file = readdir($handle)) !== false)
                {
                    if ($file{0} != '.' && filetype(APP_ROOT_PATH . $this->backup_path . $file) == 'dir')
                    {
                        $backup['name'] = $file;
                        $backup['date'] = filemtime(APP_ROOT_PATH . $this->backup_path . $file) - date('Z');
                        $backup['date_str'] = date('Y-m-d H:i:s', $backup['date']);
                        $backup['vols'] = $this->_get_vols($file);
                        $end_vol = end($backup['vols']);
                        $backup['total_size'] =round($this->_get_dir_size(APP_ROOT_PATH . $this->backup_path . $file)/1024,2);
                        $backups[] = $backup;
                    }
                }
            }
    ...
    {
        $backup_path = APP_ROOT_PATH . '/data/backup/';
    ...
    {
        return file_put_contents(APP_ROOT_PATH . $this->backup_path . $this->backup_name .
    ...
    {
        $tbl_queue_file = APP_ROOT_PATH . $this->backup_path . $this->backup_name .
            '/tbl_queue.log';
    ...
    {
        $log_file = APP_ROOT_PATH . $this->backup_path . $this->backup_name . '/vol.log';

...
{//类定义开始
    ...
    {
		require_once APP_ROOT_PATH."system/function/es_string.php";
        $randval = es_string::rand_string($length,$mode);
        es_session_set($verifyName,md5($randval));
		...
		for($i=0;$i<$length;$i++) {
			...
			ImageTTFText($im, 18, 16, $i*30+10, mt_rand(20,35), $stringColor, "public/images/t1.ttf",$randval{$i}.' ');
		}
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
	...
	static function GBVerify($length=4,$type='png',$width=180,$height=50,$fontface='simhei.ttf',$verifyName='verify') {
        ...
        es_session_set($verifyName,md5($code));
		$im=imagecreatetruecolor($width,$height);
		$borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
    ...
    static function showAdvVerify($type='png',$width=180,$height=40,$verifyName='verifyCode')
    {
        ...
        es_session_set($verifyName,$verifyCode);
        $im = imagecreate($width,$height);
        $r = array(225,255,255,223);
        $g = array(225,236,237,255);
        $b = array(225,236,166,125);

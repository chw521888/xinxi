(function() {
	$.fn.weixinAudio = function(options) {
		...
		function Plugin($context) {
			...
			this.$audio_timenow = $context.find("#audio_timenow");
			this.$audio_progress = $context.find('#audio_progress');
			this.$audio_playBall = $context.find('#audio_playBall');
			//属性
		...
		Plugin.prototype = {
			...
			animateProgressBarPosition: function() {
				...
				self.$audio_playBall.css(styles_playBall);
			},
			//每刻的播放时间
			getTimenow: function(){
				var self = this,
					timenow = self.Audio.currentTime,
					minutesnow = self.getAudioMinutes(timenow),
					secondsnow = self.getAudioSeconds(timenow),
					audioTimenow = minutesnow + ":" + secondsnow;
					self.$audio_timenow.text(audioTimenow);
			},
			//获取时间秒

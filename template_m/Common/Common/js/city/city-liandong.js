...
function comefrom(loca,locacshi) { this.loca = loca; this.locacshi = locacshi; }
...
function select(aa,bb,cc) {
if(bb==2){
if(cc==1){
document.getElementById('gw_a1').innerHTML=aa;
}else{
document.getElementById('a3_1').value=aa;
}
}else{
if(cc==1){
document.getElementById('gw_a2').innerHTML=aa;
}else{
document.getElementById('a3').value=aa;
}
}

with(document.creator.csheng) { var loca2 = options[selectedIndex].value; }
for(i = 0;i < where.length;i ++) {
if (where[i].loca == loca2) {
loca3 = (where[i].locacshi).split("|");
for(j = 0;j < loca3.length;j++) { with(document.creator.cshi) { length = loca3.length; options[j].text = loca3[j]; options[j].value = loca3[j]; var loca4=options[selectedIndex].value;}}
break;
}}
...
function init() {
with(document.creator.csheng) {
length = where.length;
for(k=0;k<where.length;k++) { options[k].text = where[k].loca; options[k].value = where[k].loca; }
options[selectedIndex].text = where[0].loca; options[selectedIndex].value = where[0].loca;
}
with(document.creator.cshi) {
loca3 = (where[0].locacshi).split("|");
length = loca3.length;
